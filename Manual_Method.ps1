...
#$final = $content | Select-Object -Skip 1 | ConvertFrom-CSV -Header $header -Delimiter '|'

#Invoke Java
try{ cd $documentpath } catch { Send-MailMessage -From $from -To $to -Subject ('Invalid Client - ' + $json.name ) -SmtpServer $smtpserver -Credential $Cred -BodyAsHtml ( $Body + "<br/>"+ "<br/>" + 'Invalid Client Supplied no Credentials Found.' ); exit }
